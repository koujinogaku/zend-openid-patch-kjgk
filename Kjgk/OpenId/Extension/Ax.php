 ...
 */
class Kjgk_OpenId_Extension_Ax extends Zend_OpenId_Extension
{
    /**
    ...
    {
        return array_keys(self::getAxPropertiesWithSchema());
    }

    public static function getAxPropertiesWithSchema()
    {
        return array(
    ...
    {
        $tproperties = self::getAxPropertiesWithSchema();
        ...
        if (is_array($this->_props) && count($this->_props) > 0) {
            ...
            $params['openid.ns.ax'] = Kjgk_OpenId_Extension_Ax::NAMESPACE_1_0;
            $params['openid.ax.mode'] = Kjgk_OpenId_Extension_Ax::MODE;
    ...
    public function parseRequest($params)
    {
        ...
        $props2 = array();
        foreach (array_keys(self::getAxPropertiesWithSchema()) as $prop) {
            if (isset($props[$prop])) {
    ...
    public function prepareResponse(&$params)
    {
        if (is_array($this->_props) && count($this->_props) > 0) {
            ...
            $params['openid.ns.ax'] = Kjgk_OpenId_Extension_Ax::NAMESPACE_1_0;
            foreach (self::getAxPropertiesWithSchema() as $prop=>$value) {
    ...
    public function parseResponse($params)
    {
        ...
        $props = array();
        foreach (self::getAxPropertiesWithSchema() as $prop=>$type) {
            if (!empty($params['openid_ax_type_' . $prop])) {
                $props[$prop] = $params['openid_ax_value_' . $prop];
            }
            else if (!empty($params['openid_ext1_type_' . $prop])) {
